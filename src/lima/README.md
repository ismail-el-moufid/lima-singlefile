...
> **Single-file build:** this tree targets Darwin/amd64 with x86_64 guests only.
> QEMU, templates, the guest agent, and firmware are built in; no `share/lima`
> directory is required. See [single-file usage and native linking](docs/single-file.md).
> The upstream platform and installation guidance below describes standard Lima.
...
To create an instance "default" from the embedded Fedora template:
```console
$ limactl start --name=default template://fedora
```
