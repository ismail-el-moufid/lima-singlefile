...
func chooseNextCreatorState(st *creatorState) (*creatorState, error) {
	for {
		...
		case prompt.Options[1]: // "Open an editor ..."
			...
			hdr += "\n"
			warning, err := editutil.GenerateEditorWarningHeader()
			if err != nil {
				return st, err
			}
			hdr += warning
		...
		case prompt.Options[2]: // "Choose another example..."
			...
			example := examples[ansEx]
			st.instName, err = guessarg.InstNameFromYAMLPath(example.Name + ".yaml")
			if err != nil {
				return nil, err
			}
			st.yBytes, err = templatestore.Read(example.Name)
			if err != nil {
				return nil, err
			}
			continue
