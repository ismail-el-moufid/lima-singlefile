...
func waitForHostAgentTermination(ctx context.Context, inst *store.Instance, begin time.Time) error {
	...
	// The exiting event precedes host-agent cleanup and process termination.
	// Wait on the original PIDs, even if their pidfiles have already been removed.
	return waitForInstanceProcesses(ctx2, inst)
}

func waitForInstanceProcesses(ctx context.Context, inst *store.Instance) error {
	processes := []struct {
		name string
		pid  int
	}{
		{"host agent", inst.HostAgentPID},
		{"QEMU", inst.QemuPID},
	}
	...
	for {
		...
		for i := range processes {
			...
			err := osutil.SysKill(process.pid, 0)
			switch {
			...
			case err == nil || errors.Is(err, os.ErrPermission):
				allStopped = false
			default:
				return fmt.Errorf("checking %s process %d during shutdown: %w", process.name, process.pid, err)
			}
		}
		if allStopped {
			return nil
		...
		select {
		case <-ctx.Done():
			return fmt.Errorf("waiting for host agent and QEMU processes to terminate: %w", ctx.Err())
		case <-ticker.C:
		}
	}
}
