...
type Result struct {
	Status          Status
	CachePath       string // "<LIMA_HOME>/_cache/download/by-url-sha256/<SHA256_OF_URL>/data"
...
// CacheDir resolves the default cache lazily from the selected Lima storage.
func CacheDir() (string, error) {
	home, err := dirnames.LimaDir()
	if err != nil {
		return "", err
	}
	return filepath.Join(home, "_cache"), nil
}

// WithCache enables caching in <resolved Lima storage>/_cache.
func WithCache() Opt {
