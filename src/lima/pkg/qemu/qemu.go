...
func EnsureDisk(cfg Config) error {
	if err := embeddedassets.CheckArch(*cfg.LimaYAML.Arch); err != nil {
		return err
	}
...
func inspectFeatures(exe string, prefix []string) (*features, error) {
	var (
		f      features
		stdout bytes.Buffer
		stderr bytes.Buffer
	)
	cmd := command.New(exe, prefix, "-M", "none", "-accel", "help")
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		return nil, fmt.Errorf("failed to run %v: stdout=%q, stderr=%q", cmd.Args, stdout.String(), stderr.String())
	}
	f.AccelHelp = append([]byte(nil), stdout.Bytes()...)
	// on older versions qemu will write "help" output to stderr
	if len(f.AccelHelp) == 0 {
		f.AccelHelp = append([]byte(nil), stderr.Bytes()...)
	}

	stdout.Reset()
	stderr.Reset()
	cmd = command.New(exe, prefix, "-M", "none", "-netdev", "help")
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		logrus.Warnf("failed to run %v: stdout=%q, stderr=%q", cmd.Args, stdout.String(), stderr.String())
	} else {
		f.NetdevHelp = append([]byte(nil), stdout.Bytes()...)
		if len(f.NetdevHelp) == 0 {
			f.NetdevHelp = append([]byte(nil), stderr.Bytes()...)
		}
	}

	stdout.Reset()
	stderr.Reset()
	cmd = command.New(exe, prefix, "-machine", "help")
	cmd.Stdout = &stdout
	cmd.Stderr = &stderr
	if err := cmd.Run(); err != nil {
		logrus.Warnf("failed to run %v: stdout=%q, stderr=%q", cmd.Args, stdout.String(), stderr.String())
	} else {
		f.MachineHelp = append([]byte(nil), stdout.Bytes()...)
		if len(f.MachineHelp) == 0 {
			f.MachineHelp = append([]byte(nil), stderr.Bytes()...)
		}
	}
...
func Cmdline(cfg Config) (string, []string, error) {
	...
	features, err := inspectFeatures(exe, args)
	if err != nil {
		return "", nil, err
	}
	...
	if !legacyBIOS && *y.Arch != limayaml.RISCV64 {
		firmware, err := firmwareForCommand(exe, args, *y.Arch)
		if err != nil {
			return "", nil, err
		}
...
func firmwareForCommand(exe string, prefix []string, arch limayaml.Arch) (string, error) {
	if command.IsBundledSystem(prefix) {
		if err := embeddedassets.CheckArch(arch); err != nil {
			return "", err
		}
		return "builtin:edk2-x86_64-code.fd", nil
	}
	return getFirmware(exe, arch)
}
