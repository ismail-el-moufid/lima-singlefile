...
func Reconcile(ctx context.Context, newInst string) error {
	...
	for _, instName := range instances {
		...
		if err != nil {
			// Deleted instances can leave sockets behind or disappear during the scan.
			if instName != newInst && errors.Is(err, os.ErrNotExist) {
				logrus.Debugf("Skipping missing instance %q during network reconciliation: %v", instName, err)
				continue
			}
			return err
		}
