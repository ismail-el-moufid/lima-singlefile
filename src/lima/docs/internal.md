...
## Lima cache directory (`<LIMA_HOME>/_cache`)

The single-file build resolves the cache lazily from the selected Lima storage:
`$LIMA_HOME`, the directory saved in `~/goinfre/lima-config.json`, or
`~/goinfre/lima-home`. `limactl prune` removes this same `_cache` directory.
Existing platform caches are neither migrated nor removed. `WithCacheDir` remains
an explicit library override (an empty directory disables caching); platform cache
environment variables do not relocate Lima's default cache.

### Download cache (`<LIMA_HOME>/_cache/download/by-url-sha256/<SHA256_OF_URL>`)
...
- `$LIMA_HOME`: The "Lima home directory" (see above).
  - Default: saved `home` in `~/goinfre/lima-config.json`, otherwise `~/goinfre/lima-home`; interactive setup remains lazy.
...
- `$QEMU_SYSTEM_X86_64`: path of `qemu-system-x86_64`
  - Default: this limactl executable with `--internal-qemu`. An explicit override may include shell-quoted arguments, which are also used for capability probes.

- `$QEMU_IMG`: optional external `qemu-img` command (including shell-quoted arguments).
  - Default: this limactl executable with `--internal-qemu-img`.

Only x86_64 guests are available in this single-file bundle. Bundled UEFI uses
`builtin:edk2-x86_64-code.fd`; explicit external QEMU retains filesystem firmware
lookup. Native workers bypass Lima settings and Cobra, reject daemonization and
memory preallocation, and leave stdout exclusively to QEMU. See `single-file.md`
for native link requirements.
