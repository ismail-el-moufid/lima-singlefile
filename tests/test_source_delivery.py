...
class SourceDeliveryTests(unittest.TestCase):
    def test_actual_package_bytes_and_original_pins(self):
        manifest = package.verify(ROOT, with_inputs=True)
        ...
        selected = {(i['module'], i['version']) for i in plan['items'] if i['kind'] == 'go-module'}
        notices = package.load(ROOT / 'compliance/notices/manifest.json')
        expected = {(m['path'], m['version']) for b in notices['binaries'].values() for m in b['modules']}
        self.assertEqual(selected, expected)
        host = next(i for i in plan['items'] if i['kind'] == 'host-go-source-subset')
        self.assertNotEqual(host['destination'].split('/')[-1], host['origin'].split('/')[-1])
        ...
        self.assertTrue(any(m['path'].startswith('go/src/runtime/') for m in provenance['members']))
        self.assertFalse(any('/bin/' == m['path'][:5] or m['path'].startswith(('go/bin/', 'go/pkg/'))
                             or m['path'].endswith('.syso') for m in provenance['members']))
    ...
    def test_gap_status_is_explicit_and_not_clearance(self):
        ...
        # Evidence reports actual bytes, not the misleading empty ignored listing.
        for item in manifest['local_relink_inputs']:
            self.assertIs(item['exported'], False)
            self.assertEqual(item['available'], (ROOT / item['path']).is_file())
            if item['available']:
                package.check_record(ROOT / item['path'], item)
    ...
    def test_go_h1_is_computed_from_bytes_not_ziphash(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            expected = package.hash1([('example.org/m@v1.0.0/go.mod', b'module example.org/m\n')])
            ...
            self.assertNotEqual(package.hash1([('go.mod', b'a')]), package.hash1([('go.mod', b'b')]))
    ...
    def test_unsafe_tar_paths_links_special_files_and_duplicates(self):
        ...
        with tarfile.open(fileobj=data, mode='w') as t:
            ...
            t.addfile(tarfile.TarInfo('root/duplicate'))
        data.seek(0)
        with tarfile.open(fileobj=data) as t, self.assertRaisesRegex(ValueError, 'Duplicate'):
            package.tar_members(t)
    ...
    def test_refuses_different_existing_outputs_and_duplicate_json(self):
        ...
        with self.assertRaisesRegex(ValueError, 'Duplicate JSON key'):
            json.loads('{"path":"a","path":"b"}', object_pairs_hook=package.unique_object)
